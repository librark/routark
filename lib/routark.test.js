...
describe('Routark', () => {
  ...
  it('only adds routes the first time', () => {
    const basePath = '/base/'
    router.addRoutes(basePath, [
      {
        path: '',
  ...
  it('might use a custom root path as base', async () => {
    ...
    router.addRoutes('/base/', [
      ...
      {
        path: 'users/:id',
        action: async ({ params }) => { userId = params.id }
      }
    ])
  ...
  it('passes a context object to every action handler', async () => {
    let passedContext = null
    let url = null
    const globalObject = {
      location: {
        get pathname () { return url },
        get search () { return '?q=country' },
        get hash () { return '#introduction' },
        get origin () { return 'https://example.com:1234' },
        get href () { return 'https://example.com:1234?q=country' }
      },
      history: {
        pushState: (_data, _title, _url) => { url = _url }
      }
    ...
    await router.navigate('/base/users/abcd1234')

    expect(passedContext).toEqual({
      location: {
        pathname: '/base/users/abcd1234',
        search: '?q=country',
        hash: '#introduction',
        origin: 'https://example.com:1234',
        href: 'https://example.com:1234?q=country'
      },
      params: {
        id: 'abcd1234'
      }
