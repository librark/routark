export class Routark {
  ...
  async _match (source, target) {
    ...
    const baseList = new Set(targetList.slice(0, splitIndex))
    const pathList = new Set(targetList.slice(splitIndex))
  ...
  async _executePath (path) {
    ...
    const href = this._global?.location?.href
    ...
    for (const route of this._routes) {
      ...
      if (builtPath === path) {
        this.current = path
        const location = { pathname, search, hash, origin, href }
